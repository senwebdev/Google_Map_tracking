(function () {
  ...
  function CGmapPageCtrl($timeout,$interval,$scope,$http,MY_CONSTANT,baSidebarService) {
    console.log("asdasf");
    var markerCount = 0;
    var searchMarkers = [];
    var bound_val =0;
    ...
    {
      $http.post(MY_CONSTANT.url + '/customers_heat_map', {
            access_token:localStorage.getItem('access_token')
      })
          .success(function(response){
            console.log(response.data.customer_data.length);
            $scope.total_no_of_users='Live Tracking (No. Of Customers '+response.data.customer_data.length+' )';
            //$scope.$apply();
            var tasks=response.data.customer_data;
            initialize1(tasks);
          })
    ...
    function createMarker2(latlng,name,email,phone,car_name,driver_no,icon) {
      ...
      marker.content='<div class="infoWindowContent">' + 'Name:'+" "+name+'<br>Email:'+" "+email+'<br>Phone Number:'+" "+phone+'<br></div>';
