...
(function() {
    ...
    angular.module('BlurAdmin.pages.drivers.documents')
        .controller('documentsCtrl', documentsCtrl);

    function documentsCtrl($timeout,$rootScope, $scope, $http, MY_CONSTANT, ngDialog, $state, $filter) {
        $scope.title = localStorage.getItem('driverName') + "'s Documents";
        $scope.currentPage = 1;
        ...
        $scope.addEditDocType = function(doc, type) {
            ...
            // else{
            //   console.log(doc.expiry_date);
            //   console.log(new Date(doc.expiry_date));
            //   console.log(moment(doc.expiry_date).format());
            //   console.log(moment(new Date(doc.expiry_date)).format());
            //   return false;
            // }
                ...
                $http.post(MY_CONSTANT.url + '/admin/upload_driver_doc', form, {
                    ...
                    .success(function(data, status) {
                        ...
                        if (data.flag == 1302) {
                            ...
                            $rootScope.openToast('error','A valid document of this type already exist for this driver.','');
                            // $timeout(function() {
                            //    $scope.docmsg = '';
                            // }, 2500);
                        }
        ...
        $scope.validateDoc = function(docID, flag) {
            $.post(MY_CONSTANT.url + '/admin/valid_invalid_doc', {
                access_token: localStorage.getItem('access_token'),
                doc_id: docID,
                valid_flag: flag
