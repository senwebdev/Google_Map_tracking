(function () {
  ...
  angular.module('BlurAdmin.pages.track.driverTracking')
      .controller('driverGMapCtrl', driverGMapCtrl);
  function driverGMapCtrl($timeout,$rootScope,$interval,$scope,$http,MY_CONSTANT,baSidebarService) {
    console.log("asdasf");
    var markerCount = 0;
    var searchMarkers = [];
    var bound_val =0;
    ...
    $.post(MY_CONSTANT.url + '/get_all_car_types', {
        ...
        .success(function (data,status) {
          console.log('car_types', data);
          $scope.carTypes=data.car_types;
          $scope.car_type_name='All Car Types';
        });
    $scope.sortBy = function(car) {
      if(car ==0){
        $scope.drawMap(0)
      }
      else{
        $scope.drawMap(car.car_type);
        $scope.car_type_name=car.car_name;
      }
    }
    $scope.drawMap=function(a)
    {
      // if(localStorage.getItem('carType'))a=localStorage.getItem('carType');
      // else localStorage.setItem('carType',a);
      localStorage.setItem('carType',a);
      ...
      .success(function(data,status){
        // console.log(response.data.customer_data.length);
        $scope.drivers=[];
        ...
        for(var i=0;i<data.drivers.length;i++){
          for(var j=0;j<$scope.carTypes.length;j++){
            if(data.drivers[i].car_type==$scope.carTypes[j].car_type)
              data.drivers[i].car_type_name=$scope.carTypes[j].car_name;
          }

          if(a!=0&&a==data.drivers[i].car_type){
            $scope.drivers.push(data.drivers[i]);
          }
        }
        $scope.total_no_of_users='Live Tracking (No. Of Drivers '+$scope.drivers.length+' )';
        $scope.$apply();
        var tasks=$scope.drivers;
        initialize1(tasks);
      })
      ...
      $interval(function(){
          if(window.location.href.indexOf('customerTracking')>-1)
            {  $scope.drawMap(localStorage.getItem('carType'));
            }
      },30000);
      ...
      $scope.fullscreenToggle = function(){
          if($scope.fullScreen=='Full Screen'){
              ...
              $('#google-mapsDriver').css('height','90vh');
          }
          else {
              $('#pieCharts').css('display','block');
              $('#google-mapsDriver').css('height','60vh');
    ...
    var initialize1 = function(info1)
      {
          ...
          $scope.mapContainer = new google.maps.Map(document.getElementById('google-mapsDriver'), mapProp);
      ...
      for(var i=0;i<info1.length;i++){
        ...
        createMarker2(new google.maps.LatLng(info1[i].current_location_latitude,
            info1[i].current_location_longitude),info1[i].driver_name,info1[i].driver_email,info1[i].driver_mobile,info1[i].car_type_name,icon);
    ...
    function createMarker2(latlng,name,email,phone,car_type_name,icon) {
      ...
      marker.content='<div class="infoWindowContent">' + 'Name:'+" "+name+'<br>Email:'+" "+email+'<br>Mobile:'+" "+phone+'<br>Car Type:'+" "+car_type_name+'<br></div>';
